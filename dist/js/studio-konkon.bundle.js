/*!
  * Studio KonKon v1.7.0-beta (https://play.studiokonkon.com/)
...
(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.konkon = factory());
  ...
  class Sidebar extends BaseComponent {
    ...
    menuRemoveActiveAll() {
      this._element.querySelectorAll(MENU_ACTIVE_SELECTOR).forEach(item => item.classList.remove(CLASS_NAME_MENU_ACTIVE));
    }
    menuShowActive(item = this._element.querySelector(MENU_ACTIVE_SELECTOR)) {
      if (item !== null) {
        ...
        SelectorEngine.parents(item, MENU_ACTIVE_PARENT_SELECTOR).forEach(menu => menu.classList.add(CLASS_NAME_MENU_OPEN));
      }
    }
    showActive(item = this._element.querySelector(MENU_ACTIVE_SELECTOR)) {
      item = typeof item === "string" ? this._element.querySelector(item) : item;
      if (item !== null) {
    ...
    menuHideAll(menuParent = this._element) {
      menuParent.querySelectorAll(MENU_OPEN_SELECTOR).forEach(subMenus => this.menuHide(subMenus));
    }
    triggerAutoClose(event, conditions = false) {
      ...
      const dataToggle = this._element.hasAttribute("id") ? event.target.closest(`[data-bs-target="#${this._element.id}"]`) : event.target.closest('[data-bs-toggle="sidebar"]');
      ...
      if (this._element === null || event.target === document || event.target === this._element || this._element.contains(event.target) || dataToggle !== null && dataToggle.contains(event.target) || conditions) {
        return;
      }
      this.hide();
    }
    ...
    _addEventListeners() {
      ...
      EventHandler.on(this._element, EVENT_CLICK_MENU, event => {
        ...
        if (SelectorEngine.children(menu, MENU_SUB_SELECTOR).length > 0) {
          ...
          menu.classList.contains(CLASS_NAME_MENU_OPEN) ? this.menuHide(menu) : this.menuShow(menu);
        } else {
          this.menuHideAll(menu.parentElement);
        }
      });
...
//# sourceMappingURL=studio-konkon.bundle.js.map
